...
class KITTI(data.Dataset):
    def __init__(self, root_dir, split, cfg):
        ...
        if self.normal_as_image:
            logging.info("Using normal as image")
        ...
        if self.split in ["val", "test", "val_car"] and self.transform_image:
            self.ht_change   = 0.9 if self.eval_dataset == "coda" else 0.0 # KITTI to Coda in Y-direction
            self.f = 721.5377
            self.depth_plane = 2.0 if 'depth_plane' not in cfg.keys() else cfg['depth_plane']
