...
def get_calib_from_file(calib_file):
    ...
    try:
        ...
        gd_to_cam = np.array(obj, dtype=np.float32)
        if len(gd_to_cam) <= 0:
            flag = True
    except:
        flag = True
    ...
    if flag:
        # KITTI ego vehicle
        gd_to_cam = np.eye(4)[:3, :]
        gd_to_cam[1, 3] = 1.6500
        gd_to_cam = gd_to_cam.flatten()

    try:
        obj = lines[8].strip().split(' ')[1:]
        intrinsics = np.array(obj, dtype=np.float32)
    except:
        intrinsics = P2.reshape(3, 4)[:3, :3].flatten()

    return {'P2': P2.reshape(3, 4),
            'P3': P3.reshape(3, 4),
            'R0': R0.reshape(3, 3),
            'Tr_velo2cam': Tr_velo_to_cam.reshape(3, 4),
            'gd_to_cam': gd_to_cam.reshape(3, 4),
...
class Calibration(object):
    def __init__(self, calib_file):
        ...
        self.gd_to_cam = calib['gd_to_cam']
        self.intrinsics = calib['intrinsics']
