...
import numpy as np
import copy
from lib.models.sesn_lib.ses_conv import *
import logging
...
def _approximate_weight(basis, target_weight, eps=1e-12):
    ...
    if matrix_rank == basis.shape[0]:
        ...
        flag = True
        logging.info('  !!! basis is incomplete. rank={} < num_funcs={}. '
    ...
    if flag:
        logging.info('  relative_diff={:.1e}, abs_diff={:.1e}'.format(diff / norm, diff))
    x = torch.Tensor(x)
