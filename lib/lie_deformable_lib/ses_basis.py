...
import math
from lib.helpers.homography_helper import warp_images
...
def steerable_A(size, scales, homos, effective_size, **kwargs):
    ...
    num_homos       = len(homos)
    # Create homographed version of filters
    if homos is not None and num_homos > 0:
        index = -1
        ...
        basis_images = np.array(basis_tensors[index])
        # Because of projective transform, there is division by focal length
        # We fix this to 1.
        f = 1
        for homo in homos:
            # y-plane and depth movement
            output_images = warp_images(basis_images, ty= 0., tz= homo, f= f, n= 1., o= 0., k= 10.)
            basis_tensors.append(torch.tensor(output_images))

    steerable_basis = torch.stack(basis_tensors, 1)
    if homos is not None and num_homos > 0:
        # Rearrange basis
        scales_wo_index = np.delete(np.arange(num_scales), index)
        reorder         = np.concatenate((np.array([index]), num_scales+np.arange(num_homos), scales_wo_index))
        steerable_basis = steerable_basis[:, torch.tensor(reorder)]
