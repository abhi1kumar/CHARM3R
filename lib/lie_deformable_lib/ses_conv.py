...
class SESConv_Z2_H(nn.Module):
    ...
    [B, C, H, W] -> [B, C', num_homos x num_scales, H', W']
    ...
    def __init__(self, in_channels, out_channels, kernel_size, effective_size,
                 scales=[1.0], homos=[],stride=1, padding=0, bias=False, padding_mode='constant', dilation= 1, norm_per_scale= True, rescale_basis= False, **kwargs):
        super().__init__()
        ...
        self.homos = homos
        self.num_homos = len(homos)
        self.num_trans = self.num_scales + self.num_homos
        self.stride = stride
        ...
        basis = steerable_A(kernel_size, scales, self.homos, effective_size, **kwargs)
    ...
    def forward(self, x):
        ...
        kernel = kernel.view(self.out_channels, self.in_channels,
                             self.num_trans, self.kernel_size, self.kernel_size)
        ...
        B, C, H, W = y.shape
        y = y.view(B, self.out_channels, self.num_trans, H, W)
    ...
    def extra_repr(self):
        s = '{in_channels}, {out_channels}, scales={scales}, homos={homos}, size={kernel_size}, eff_size={effective_size}, stride={stride}, padding={padding}'
        return s.format(**self.__dict__)


class SESConv_H_H(nn.Module):
    '''Scale Equivariant Steerable Convolution: (S x Z2) -> (S x Z2)
    [B, C, num_homos + num_scales, H, W] -> [B, C', num_homos + num_scales, H', W']

    Args:
        in_channels: Number of channels in the input image
        out_channels: Number of channels produced by the convolution
        scale_size: Size of scale filter
        homos_size: Size of homos filter
        kernel_size: Size of the convolving kernel
        effective_size: The effective size of the kernel with the same # of params
        scales: List of scales of basis
        stride: Stride of the convolution
    ...
    def __init__(self, in_channels, out_channels, scale_size, homos_size, kernel_size, effective_size,
                 scales=[1.0], homos=[], stride=1, padding=0, bias=False, padding_mode='constant', norm_per_scale= True, rescale_basis= False, **kwargs):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.scale_size = scale_size
        self.homos_size = homos_size
        self.kernel_size = kernel_size
        self.effective_size = effective_size
        self.scales = [round(s, 3) for s in scales]
        self.num_scales = len(scales)
        ...
        self.weight = nn.Parameter(torch.Tensor(
            out_channels, in_channels, self.homos_size * self.scale_size, self.num_funcs))
        if bias:
            self.bias = nn.Parameter(torch.Tensor(out_channels))
        else:
            self.register_parameter('bias', None)
    ...
    def forward(self, x):
        ...
        basis = self.basis.view(self.num_funcs, -1)
        kernel = self.weight @ basis
        kernel = kernel.view(self.out_channels, self.in_channels, self.homos_size * self.scale_size,
                             self.num_trans, self.kernel_size, self.kernel_size)
        ...
        for i in range(self.scale_size):
            ...
            output += F.conv2d(x_, kernel[:, :, i], groups=S, stride=(self.stride, self.stride))
...
class SESConv_H_H_1x1(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, scale_size=1, homos_size=1, stride=1, num_scales=1, bias=False):
        super().__init__()
        self.in_channels = in_channels
        self.out_channels = out_channels
        self.scale_size = scale_size
        ...
        if scale_size > 1 or homos_size > 1:
            # workaround for being compatible with the old-style weight init
            self.weight = nn.Parameter(torch.Tensor(out_channels, in_channels, self.scale_size + self.homos_size, 1, 1))
        else:
    ...
    def extra_repr(self):
        s = '{in_channels}, {out_channels}, scale_size={scale_size}, homos_size= {homos_size}, kernel_size=(1, 1),  stride={stride}'
        return s.format(**self.__dict__)
