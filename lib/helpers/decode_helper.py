...
def canonical_to_raw(x, y, can2raw):
    pts = np.array([[[x, y]]], np.float32) # 1x1x2
    out = cv2.perspectiveTransform(pts, can2raw) # 1x1x2
...
def decode_detections(dets, info, calibs, cls_mean_size, threshold):
    ...
    for i in range(dets.shape[0]):  # batch
        preds = []
        raw2can = info['raw2can'][i]
        can2raw = np.linalg.inv(raw2can)
        for j in range(dets.shape[1]):  # max_dets
            ...
            x, y = canonical_to_raw(x, y, can2raw)
            ...
            x3d, y3d = canonical_to_raw(x3d, y3d, can2raw)
