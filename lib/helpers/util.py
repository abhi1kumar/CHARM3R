...
from lib.helpers.file_io import read_image, read_numpy
from lib.helpers.homography_helper import _warp_images_with_per_pixel_mat
...
def gaussian_filter_2d(img: torch.Tensor, sigma: float) -> torch.Tensor:
    ...
    c = img.shape[0]
    img = img.unsqueeze(0)  # Need 4D data for ``conv2d()``
    for i in range(c):
        # Convolve along columns and rows
        t = conv2d(img[:, i].unsqueeze(1), weight=kernel_1d.view(1, 1, -1, 1), padding=(padding, 0))
        t = conv2d(t, weight=kernel_1d.view(1, 1, 1, -1), padding=(0, padding))
        out[i] = t[0, 0]
    return out

def downsample_matrix(matrix, downsample= 1, smooth= False):
    c, h, w = matrix.shape
    if c == 3 or c == 1:
        ...
        if smooth:
            ...
            matrix = gaussian_filter_2d(matrix, sigma= downsample/2)

            if NUMPY:
                matrix = matrix.cpu().detach().numpy().float()

        return matrix[:, ::downsample, ::downsample]
    else:
        return matrix[::downsample, ::downsample, :]

def read_RGB(img_path, device= None, downsample= 1, smooth= False):
    img = read_image(path= img_path, rgb= True)
    if device is not None:
        img = np.array(img)/255.
        img   = torch.from_numpy(img).float().permute(2, 0, 1).to(device)  # 3 x h x w

    if downsample > 1:
        img = downsample_matrix(img, downsample, smooth)

    return img

def read_depth(depth_path, device= None, downsample= 1, smooth= False):
    depth = read_numpy(depth_path, show_message=False)
    if device is not None:
        depth = torch.from_numpy(depth).float().to(device).unsqueeze(0)  # 1 x h x w

    if downsample > 1:
        depth = downsample_matrix(depth, downsample, smooth)

    return depth

def read_normal(normal_path, device= None, downsample= 1, smooth= False):
    """Normals in X, Y, Z as KITTI Image Convention"""
    ...
    if downsample > 1:
        normal = downsample_matrix(normal, downsample, smooth)

    return normal

def my_print(u, v, points, points_3d, normal, depth, K, homography= None, homo2= None, threeD_first= True):
    if threeD_first:
        print("Pts2D  :", points[:, v, u])
        print("Pts3D  :", points_3d[:, v, u])
        print("Depth  :", depth[v, u])
        print("Normal :", normal[:, v, u])
        print("K      :", K[v, u])
        print("Plane  :", torch.sum(normal[:, v, u] * points_3d[:, v, u]) + K[v, u])
    else:
        print("Pts2D  :", points[v, u])
        print("Pts3D  :", points_3d[v, u])
        print("Depth  :", depth[v, u])
    ...
    if homography is not None:
        print("Homo   :", homography[v, u])
        print("Homo Inv :", torch.linalg.inv(homography[v, u]))
...
def ee_error(img0, img1, mask= None):
    EPS = 1e-3
    if type(img0) == np.ndarray:
        ...
        img0 = img0[min_y: max_y, min_x: max_x]
        img1 = img1[min_y: max_y, min_x: max_x]

    elif type(img0) == torch.Tensor:
        pass
    ...
    return my_norm(img0 - img1) / (my_norm(img1)  + EPS)

def pass_over_model(img0, homo_img, img0_t, model, mask= None):
    with torch.no_grad():
        # Transform and Pass image
        f0    = model(img0)     # 1 x c x h x w
        f0_t  = _warp_images_with_per_pixel_mat(f0, homo_img.unsqueeze(0))   # 1 x c x h x w

        # Pass transformed_image
        f1    = model(img0_t)     # 1 x c x h x w

    return f0_t, f1, ee_error(img0= f0_t, img1= f1, mask= mask).item()