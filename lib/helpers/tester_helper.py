...
class Tester(object):
    def __init__(self, cfg, model, data_loader, logger, results_test= 'result_test'):
        ...
        if self.eval_dataset == "carla":
            ...
            results_test    = os.path.join(results_test, self.ext_config)
        ...
        self.eval_adapt       = False                             if 'eval_adapt'       not in cfg['model'].keys() else cfg['model']['eval_adapt']
        self.shift_coord_conv = [[1, 0, 0], [0, 1, 0], [0, 0, 1]] if 'shift_coord_conv' not in cfg['model'].keys() else cfg['model']['shift_coord_conv']
    ...
    def test(self):
        ...
        # Adapt
        if self.eval_adapt and (self.shift_coord_conv is not None or self.eval_gd_homo is not None):
            method = getattr(self.model, 'adapt_in_eval', None)
            if callable(method):
                self.model.adapt_in_eval(self.shift_coord_conv , self.eval_gd_homo)
        ...
        for batch_idx, (inputs, calibs, coord_ranges, targets, info) in enumerate(self.data_loader):
            ...
            outputs = self.model(inputs,coord_ranges,calibs,targets,info,K=50,mode='test')
        ...
        if self.eval_adapt and (self.shift_coord_conv is not None or self.eval_gd_homo is not None):
            method = getattr(self.model, 'adapt_in_eval', None)
            if callable(method):
                self.model.reset_adapt_in_eval()
        ...
        if self.eval_dataset == "kitti":
            ...
            gt_folder = os.path.join("data/carla", self.ext_config, "validation/label")
            print(gt_folder)
