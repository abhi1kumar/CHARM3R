...
class Trainer(object):
    def __init__(self,
        ...
        self.clip_gradient = cfg['optimizer']['clip_gradient'] if 'clip_gradient' in cfg['optimizer'].keys() else False
        self.clip_value    = cfg['optimizer']['clip_value']    if 'clip_value'    in cfg['optimizer'].keys() else 10.0
        self.model = model
        ...
        if self.eval_dataset == "carla":
            self.ext_config = 'pitch0' if 'ext_config' not in self.cfg['dataset'].keys() else self.cfg['dataset']['ext_config']
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.cfg_train['max_epoch']):
            ...
            self.logger.info('Name= {} '.format(os.path.basename(self.cfg_train['log_dir'])))
    ...
    def train_one_epoch(self,loss_weights=None):
        ...
        for batch_idx, (inputs,calibs,coord_ranges, targets, info) in enumerate(self.train_loader):
            ...
            if self.clip_gradient:
                ...
                torch.nn.utils.clip_grad_norm(parameters= self.model.parameters(), max_norm= self.clip_value, norm_type= 2.0)
    ...
    def eval_one_epoch(self):
        self._eval_one_epoch(self.test_loader)
        if self.test_loader2 is not None:
            self._eval_one_epoch(self.test_loader2)

    def _eval_one_epoch(self, eval_loader):
        ext_config = eval_loader.dataset.ext_config if hasattr(eval_loader.dataset, 'ext_config') else None
        self.model.eval()
        results = {}
        disp_dict = {}
        progress_bar = tqdm.tqdm(total=len(eval_loader), leave=True, desc='Evaluation Progress')
        with torch.no_grad():
            for batch_idx, (inputs, calibs, coord_ranges, targets, info) in enumerate(eval_loader):
                ...
                outputs = self.model(inputs,coord_ranges,calibs,targets,info,K=50,mode='val')

                dets = extract_dets_from_outputs(outputs, K=50)
                dets = dets.detach().cpu().numpy()
                
                # get corresponding calibs & transform tensor to numpy
                calibs = [eval_loader.dataset.get_calib(index)  for index in info['img_id']]
                info = {key: val.detach().cpu().numpy() for key, val in info.items()}
                cls_mean_size = eval_loader.dataset.cls_mean_size
        ...
        if ext_config is not None:
            output_folder  = os.path.join(output_folder, ext_config)
        ...
        elif self.eval_dataset == "carla":
            gt_folder = os.path.join("data/carla", ext_config, "validation/label")
        elif self.eval_dataset == "coda":
            gt_folder = "data/coda/training/label_2"
        else:
