...
class GupnetLoss(nn.Module):
    ...
    def forward(self, preds, targets, task_uncertainties=None):
        ...
        lie_loss    = self.compute_lie_segmentation_loss(preds, targets)

        loss = seg_loss + bbox2d_loss + bbox3d_loss + lie_loss
        
        return loss, self.stat

    def compute_lie_segmentation_loss(self, input, target):
        if 'lie_wt' in input.keys() and input['lie_wt'] is not None:
            input['lie_wt']       = torch.clamp(input['lie_wt'], min= 1e-4, max= 1 - 1e-4)[:, 0] # B x H x W
            target_lie_seg        = torch.zeros_like(input['lie_wt']).cuda() # B x H x W
            mask                  = target['seman'] == 1
            _, h, w               = input['lie_wt'].shape
            _, H, W               = target['seman'].shape
            downsample            = H//h
            mask                  = mask[:, ::downsample, ::downsample]
            target_lie_seg[mask]  = 1
            loss                  = 0.1 * F.binary_cross_entropy(input['lie_wt'], target_lie_seg)
        else:
            loss                  = torch.tensor(0.0).to(target['heatmap'].device)
