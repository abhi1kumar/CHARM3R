"""
    Sample Run:
    python generate_multiple_yaml_sb.py -i experiments/run_11.yaml -t 10 -c 5 --no_sb
...
def get_sb_file_content(yaml_file_path, yaml_basename, time_in_hour, index=0):
    ...
    content += "#SBATCH --mem-per-cpu=8G            # memory required per allocated CPU (or core) - amount of memory (in bytes)\n"
    if index == 0:
        content += "#SBATCH --job-name " + yaml_basename.replace("run_", "")  + "            # you can give your job a name for easier identification (same as -J)\n"
    else:
        content += "#SBATCH --job-name " + yaml_basename.replace("run_", "")  + "_" + str(index) + "            # you can give your job a name for easier identification (same as -J)\n"
    ...
    content += "conda activate DEVIANT ### Activate virtual environment\n"
    content += "srun /mnt/home/kumarab6/anaconda3/envs/DEVIANT/bin/python -u tools/train_val.py --config=" + yaml_file_path + " ### Run python code\n"
...
ap.add_argument('-t', '--time' ,  type= int, default= '20', help= "running time in hours")
ap.add_argument('-c', '--copies', type= int, default= 3   , help= 'number of copies of experiments')
ap.add_argument('--no_sb', action='store_true', default=False, help= 'do not generate sb files')
args    = ap.parse_args()
...
for i in range(1, total_copies):
    ...
    # replace seed
    new_seed = 111*i if i<=3 else 111*(i+1)
    lines = read_lines(yaml_paths_list[i], strip= False)
    for l, line in enumerate(lines):
        if "random_seed" in line:
            lines[l] = "random_seed: " + str(new_seed) + "\n"
    write_lines(yaml_paths_list[i], lines)
