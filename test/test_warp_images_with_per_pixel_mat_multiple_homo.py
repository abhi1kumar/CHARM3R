...
from lib.helpers.util import init_torch, read_RGB, read_depth, read_normal, my_print, inch_2_meter, ee_error
from lib.helpers.homography_helper import get_intrinsics_from_fov, sample_points_on_plane, get_per_pixel_image_homo, _warp_images_with_per_pixel_mat, invert_Homography
...
def foo(folder, ty_inch= -30, indices= None):
    ...
    for i, (imf, df, nf, sf) in enumerate(zip(image_files, depth_files, normal_files, seman_files)):
        ...
        image  = read_RGB(imf, device, downsample)
        depth  = read_depth(df, device, downsample)  # 1 x h x w
        normal = read_normal(nf, device, downsample) # 3 x h x w
        seman  = read_depth(sf, device, downsample)  # 3 x h x w
        ...
        depth1 = read_depth(trn_depth_path, device, downsample)  # 1 x h x w
        normal1 = read_normal(trn_normal_path, device, downsample) # 3 x h x w
        ...
        with torch.no_grad():
            if invert_Homo:
                Hom01_pts = get_per_pixel_image_homo(depth, normal, tx = 0., ty= -ty, tz = 0., fov_degree= fov_degree, method= "points")[0]
                Hom10_pts = invert_Homography(Hom01_pts, depth)
                Hom01_inf = get_per_pixel_image_homo(depth, normal, tx = 0., ty= -ty, tz = 0., fov_degree= fov_degree, method= "infinite_plane")[0]
                Hom10_inf = invert_Homography(Hom01_inf, depth)
                Hom01_sam = get_per_pixel_image_homo(depth, normal, tx = 0., ty= -ty, tz = 0., fov_degree= fov_degree, method= "plane_sampling")[0]
                Hom10_sam = invert_Homography(Hom01_sam, depth)
            else:
                Hom10_pts = get_per_pixel_image_homo(depth, normal, tx = 0., ty= ty, tz = 0., fov_degree= fov_degree, method= "points")[0]
                Hom10_inf = get_per_pixel_image_homo(depth, normal, tx = 0., ty= ty, tz = 0., fov_degree= fov_degree, method= "infinite_plane")[0]
                Hom10_sam = get_per_pixel_image_homo(depth, normal, tx = 0., ty= ty, tz = 0., fov_degree= fov_degree, method= "plane_sampling")[0]
            ...
            img0_warp_pts_t = _warp_images_with_per_pixel_mat(img0_tensor, Hom10_pts.unsqueeze(0))        # b x 3 x h x w
            img0_warp_pts   = img0_warp_pts_t.cpu().float()[0].permute(1, 2, 0).numpy().astype(np.uint8)  # h x w x 3

            img0_warp_inf_t = _warp_images_with_per_pixel_mat(img0_tensor, Hom10_inf.unsqueeze(0))        # b x 3 x h x w
            img0_warp_inf   = img0_warp_inf_t.cpu().float()[0].permute(1, 2, 0).numpy().astype(np.uint8)  # h x w x 3

            img0_warp_sam_t = _warp_images_with_per_pixel_mat(img0_tensor, Hom10_sam.unsqueeze(0))        # b x 3 x h x w
        ...
        if show_points:
            ...
            savefig(plt, "images/warp_images_with_per_pixel_homo_multiple_homo.png")
            plt.show()
            plt.close()
